...
- [[#Dashboard — `src/apps/kipo-dashboard/.env.local`|Dashboard — `src/apps/kipo-dashboard/.env.local`]]
- [[#Backend — `src/apps/kipo-platform/.env`|Backend — `src/apps/kipo-platform/.env`]]
	- [[#Backend — `src/apps/kipo-platform/.env`#Supabase|Supabase]]
	- [[#Backend — `src/apps/kipo-platform/.env`#Stripe (pagos y suscripciones)|Stripe (pagos y suscripciones)]]
	- [[#Backend — `src/apps/kipo-platform/.env`#CORS|CORS]]
	- [[#Backend — `src/apps/kipo-platform/.env`#Flask|Flask]]
- [[#Raíz — `.env` (providers de auth de Supabase)|Raíz — `.env` (providers de auth de Supabase)]]
- [[#Resumen por entorno|Resumen por entorno]]
...
| Variable                      | Local                                                     |
| ----------------------------- | --------------------------------------------------------- |
| `PROJECT_URL`                 | `http://127.0.0.1:54321` (Supabase local)                 |
| `DATABASE_URL`                | `postgresql://postgres:postgres@127.0.0.1:54322/postgres` |
| `CORS_WILDCARD_DOMAIN`        | `localhost`                                               |
| `AUTH_KEY_PUBLISHABLE`        | anon key Supabase local                                   |
| `AUTH_KEY_SECRET`             | service_role key Supabase local                           |
| `STRIPE_PUBLISHABLE`          | `pk_test_...`                                             |
| `STRIPE_SECRET_KEY`           | `sk_test_...`                                             |
| `STRIPE_WEBHOOK_SECRET`       | `whsec_...` (stripe listen)                               |
| `STRIPE_STAMP_WEBHOOK_SECRET` | `whsec_...` (stripe listen)                               |
| `STRIPE_PRICE_EMPRENDEDOR`    | `price_1TxaieEC4...`                                      |
| `STRIPE_PRICE_PYME`           | `price_1TxajIEC4...`                                      |
